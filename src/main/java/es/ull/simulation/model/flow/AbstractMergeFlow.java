...
/**
 * A IFlow which merges several incoming branches into a single outgoing branch. The incoming 
 ...
 * are computed. Then, the IFlow checks (depending on the implementation) if this branch 
 * <code>canPass</code> or not. If it can pass, the outgoing IFlow is activated. Next, 
 * the IFlow checks if the branch <code>canReset</code> the structure, if so, it's reset.
 ...
 * @author ycallero
 */
public abstract class AbstractMergeFlow extends AbstractSingleSuccessorFlow implements IJoinFlow {
	...
	/**
	 * Create a new AbstractMergeFlow intended to be used in a safe context.
	 * @param model The simulation model this IFlow belongs to
	 */
	public AbstractMergeFlow(final Simulation model) {
		this(model, true);
	}

	/**
	 * Create a new AbstractMergeFlow which can be used in a safe context or a general one.
	 * @param safe True for safe context; false in other case
	 */
	public AbstractMergeFlow(final Simulation model, final boolean safe) {
		super(model);
		this.safe = safe;
	}
	...
	/**
	 * Returns how many incoming branches this IFlow has.
	 * @return How many incoming branches this IFlow has
	 */
	...
	/**
	 * Returns the safety of this IFlow. 
	 * @return The safety of this IFlow
	 */
	...
	/**
	 * Returns a {@link MergeFlowControl} instance to control the behaviour of the IFlow
	 * @return a {@link MergeFlowControl} instance to control the behaviour of the IFlow
	 */
